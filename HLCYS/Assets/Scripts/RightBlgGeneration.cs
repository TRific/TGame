...
public class RightBlgGeneration : MonoBehaviour {
    ...
    void Update()
    {
        ...
        //if (myBld.transform.position.z > 260.0f && myBld.transform.position.z < 262.0f)
        {
            ...
            if(n == 0)
            {
                var go =Instantiate(Resources.Load("FullBuilding_1",typeof(GameObject)), startGenerationPoint1, Quaternion.identity) as GameObject;
                go.AddComponent<RightBlgGeneration>();
                Destroy(gameObject);
            }else if(n == 1){
                var go = Instantiate(Resources.Load("FullBuilding_2", typeof(GameObject)), startGenerationPoint1, Quaternion.identity) as GameObject;
                go.AddComponent<RightBlgGeneration>();
                Destroy(gameObject);
            }
            else if(n == 2){
                var go = Instantiate(Resources.Load("HalfBuildings_1", typeof(GameObject)), startGenerationPoint1, Quaternion.identity) as GameObject;
                go.AddComponent<RightBlgGeneration>();
                Destroy(gameObject);
            }
            else
            {
                var go = Instantiate(Resources.Load("TripleBuildings_1", typeof(GameObject)), startGenerationPoint1, Quaternion.identity) as GameObject;
                go.AddComponent<RightBlgGeneration>();
                Destroy(gameObject);
            }
